...
class LatentSpaceObjective:
    ...
    def __call__(self, z):
        ...
        scores = np.array([np.nan]*len(decoded_xs))
        querry_idx=[]
        querry_ls = []
        for i,x in enumerate(decoded_xs):
            if x not in self.xs_to_scores_dict:
                querry_idx.append(i)
                querry_ls.append(x)
            else:
                scores[i] = self.xs_to_scores_dict[x]
        querry_ = self.query_oracle(querry_ls)
        print(len(querry_ls))
        for i in range(len(querry_idx)):
            self.xs_to_scores_dict[decoded_xs[querry_idx[i]]] = querry_[i]
        print(len(self.xs_to_scores_dict.keys()))
        scores[querry_idx] = querry_
        self.num_calls+= np.sum(np.logical_not(np.isnan(querry_)))
